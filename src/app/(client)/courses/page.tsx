...
export default async function Courses() {
  ...
  const { data: technologias } = await supabase
    .from('technologias')
    .select('*');
  ...
  return (
    <main className="h-auto" role="main">
      <Header
        ...
        paragraph="Encuentra los mejores cursos gratuitos dictados por los mejores creadores de contenido de programación, diseño y ciberseguridad"
      />

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
        {cursos?.map((curso) => (
          <article
            className="border w-full bg-[#13203E] flex flex-col"
            role="article"
            key={curso.id}>
            ...
            <main className="p-4 flex-1">
              ...
              <div className="tags flex mt-4 gap-2 text-xs">
                <div className="flex gap-2">
                  {curso.tecnologia1 ? (
                    <span className="bg-[#978d2e] text-white p-2 rounded-2xl">
                      {curso.tecnologia1}
                    </span>
                  ) : null}
                  {curso.tecnologia2 ? (
                    <span className="bg-[#2b2c88] text-white p-2 rounded-2xl">
                      {curso.tecnologia2}
                    </span>
                  ) : null}
                  {curso.categoria ? (
                    <span className="bg-[#852929] text-white p-2 rounded-2xl">
                      {curso.categoria}
                    </span>
                  ) : null}
                </div>
